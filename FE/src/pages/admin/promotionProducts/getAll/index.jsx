  ...
  const handleDelete = async (id) => {
    ...
    return (
      <div className="container mx-auto p-4 bg-white shadow rounded">
        ...
        {/* Nếu bạn đã có FormDelete, có thể dùng lại ở đây */}
        
        {selectedPromotionProduct && (
          <FormDelete
            isOpen={true}
            onClose={() => setSelectedPromotionProduct(null)}
            onConfirm={() => handleDelete(selectedPromotionProduct.id)}
            message={`Bạn có chắc chắn muốn xóa sản phẩm khuyến mãi ID ${selectedPromotionProduct.id}?`}
          />
        )}

...
function EditBlog() {
  ...
  useEffect(() => {
    if (blogCategoryId && categories.length > 0) {
      const selectedCategory = categories.find((cat) => cat.id === blogCategoryId);
      if (selectedCategory) {
        setSelectedCategoryName(selectedCategory.name);
      }
    }
  }, [blogCategoryId, categories]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!metaDescription.trim()) {
      newErrors.metaDescription = "Mô tả không được để trống.";
    } else if (metaDescription.trim().length > 500) {
      newErrors.metaDescription = "Mô tả không được vượt quá 500 ký tự.";
    }
              ...
              <form onSubmit={handleSubmit}>
                ...
                <Editor
                  ...
                  init={{
                    height: 200,
                    menubar: false,
                    plugins: ["link", "lists", "autolink", "charmap", "preview"],
